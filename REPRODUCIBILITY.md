...
Документ описывает три уровня проверки: аудит опубликованных таблиц без GPU, повторную оценку лучшей контрольной точки и полное обучение с нуля.

## 1. Требования

- Python 3.10 или новее;
- для обучения и полной оценки рекомендуется GPU NVIDIA с поддержкой `float16`;
- примерно 30 ГБ свободного места для окружения, данных и контрольных точек;
...
## 3. Загрузка лучшей контрольной точки
...
Для совпадения с опубликованными числами нужен исходный `val.bin`: 1 000 000 токенов `uint16`, то есть 2 000 000 байт. Этот файл не опубликован в Git или модельном репозитории. Если у вас есть исходный пакет эксперимента, поместите файл в `data/val.bin`; иначе подготовьте новый поток по следующему разделу и рассматривайте результат как повторение протокола, а не точное численное совпадение.

```bash
mkdir -p data
# Поместите исходный или заново подготовленный val.bin в data/val.bin.
...
Оценщик создаёт генератор со случайным зерном 1234 отдельно для каждой глубины, поэтому все глубины получают одни и те же пакетные выборки. Опубликованные контрольные значения для зерна 43:
...
При исходном бинарном потоке небольшие различия последнего десятичного знака возможны из-за версии CUDA и режима вычислений. На заново подготовленном потоке абсолютные значения могут отличаться; проверять следует относительное преимущество и форму кривой.

## 5. Подготовка данных для обучения с нуля

Исходный токенизатор Qwen не используется: его словарь нарушил бы ограничение на число параметров. Сначала обучается байтовый BPE-словарь размером 16 000:
...
- промежуточные потери на глубине 8, в средней точке и на конечной глубине;
- веса 0,25 / 0,25 / 0,5;
...
outputs/experiments/007_multiseed/fixed_ma_seed43/
  best_hf/          лучшая контрольная точка
...
## 7. Повторение парного трёхзернового сравнения

Для стандартного порядка используйте конфигурации эксперимента 005, для MLP → Attention — конфигурации эксперимента 007. Требуются зерна 42, 43 и 44. Не меняйте данные, бюджет, критерий выбора контрольной точки или набор глубин проверки.

После обучения выполните плотную оценку каждой контрольной точки на глубинах 4–32 и по 100 пакетных выборок. Итоговые эталонные таблицы находятся в:
...
Критерий успешного повторения — преимущество MLP → Attention у каждого зерна и на каждой глубине. В исходном прогоне получено 87 побед из 87.
...
- Оценка публичной контрольной точки на исходном `val.bin`, если он сохранён у исполнителя, воспроизводит итоговые метрики.
- Обучение на исходных бинарных потоках проверяет полный запуск.
- Повторная потоковая выгрузка FineWeb проверяет метод, но не является строгой репликой данных без закреплённой версии и порядка документов.
