#!/usr/bin/env python3
"""Create a compact Experiment 004 bundle without large checkpoint weights."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bundle Experiment 004 results")
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "experiment_004_results.zip")
...
def main() -> None:
    ...
    for variant in VARIANTS:
        run_dir = REPO_ROOT / "outputs" / "experiments" / "004" / variant
        candidates.extend(
            run_dir / name
            for name in (
                "metrics.jsonl",
